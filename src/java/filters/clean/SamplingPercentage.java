 ...
 * @author santos
 */
public class SamplingPercentage extends UtilFunctions {
    ...
    private RemovePercentage filterRemovePercentage = new RemovePercentage();
    private DinamicTable dinamicTable = new DinamicTable();
    private double percentage = 50.0;
    private boolean invertSelection = false;

    public SamplingPercentage(Node p, GraphicControlMB g) {
        currentNode = p;
        graphicControlMB = g;
        reset();
    }
    ...
    public final void reset() {
        ...
        submenu.addElement(createDefaultMenuItem("Configure", Boolean.FALSE, "PF('wvContextMenu').hide();", "#{graphicControlMB.selectedNode.dataSamplingPercentage.showConfigure}", "Configure", "", "fa fa-cogs"));
        submenu.addElement(createDefaultMenuItem("Run", Boolean.TRUE, "PF('wvContextMenu').hide();", "#{graphicControlMB.selectedNode.dataSamplingPercentage.runProcess}", "Run process", ":IdFormDialogsUpdateMissing", "fa fa-cogs"));
        submenu.addElement(createDefaultMenuItem("View", Boolean.TRUE, "PF('wvDlgViewSamplingPercentage').show(); PF('wvContextMenu').hide();", null, "View data ", ":IdFormDialogsSamplingPercentage", "fa fa-search"));
        ...
        submenu.addElement(createDefaultMenuItem("Help", Boolean.FALSE, "PF('wvDlgSamplingPercentageHelp').show(); PF('wvContextMenu').hide();", null, "Help", "", "fa fa-question-circle"));
    ...
    public void showConfigure() {
        ...
        RequestContext.getCurrentInstance().update("IdFormDialogsSamplingPercentage");
        RequestContext.getCurrentInstance().execute("PF('wvDlgSamplingPercentage').show()");
    }
    ...
    public void saveConfiguration() {
        ...
        RequestContext.getCurrentInstance().execute("PF('wvDlgSamplingPercentage').hide()");
    ...
    public void runProcess() {
        try {
            ...
            filterRemovePercentage.setInvertSelection(!invertSelection);
            filterRemovePercentage.setPercentage(percentage);
            filterRemovePercentage.setInputFormat(data);
            data = Filter.useFilter(data, filterRemovePercentage);//System.out.println(data);
